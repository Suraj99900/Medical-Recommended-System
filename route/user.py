...
# load the modal and return the data 
def predectWithSVCModel(patientSymptoms):
    try:
        ...
        for item in patientSymptoms:
            if item in symptoms_dict:
                ...
                raise ValueError(f"Symptom '{item}' not found in symptoms dictionary")
        
        # Predict the index of the disease
        predicted_index = svc.predict([input_vector])[0]
        
        # Return the decoded disease
        return decodedDiseasesDict.get(predicted_index, "Disease not found")
...
def helper(sPredictedDisease):
    ...
    sDiets = sDiets.values.tolist()  # Convert to list

    sWorkout = workout[workout['disease'] == sPredictedDisease]['workout']
    sWorkout = sWorkout.values.tolist()  # Convert to list
...
def predict():
    if request.is_json:
        ...
        if symptoms:
            sUserSymptoms = [re.sub(r'\s+', '_', s.strip()) for s in symptoms.split(',')]
            sPredictedDisease = predectWithSVCModel(sUserSymptoms)

            aReturnData = helper(sPredictedDisease)

            return jsonify(aReturnData)  # Use jsonify to ensure JSON response
        else:
